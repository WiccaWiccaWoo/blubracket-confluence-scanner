...
class ConfluenceRiskScanner:
    ...
    def scan_all_spaces(self, out):
        print("\n RETRIEVING ALL SPACE KEYS...\n")

        space_count = 0
        space_keys = []
        
        # GET requests to the Confluence API are limited to 1000 records per request.
        # If the below request stops at 1000 records, this loop will perform another requests. This will repeat until the 
        # request is stopped at any number lower than 1000.

        while space_count % 1000 == 0:
            space_data = self.client.get_all_spaces(start=space_count, limit=10000, expand=None)
            for data in space_data['results']:
                space_keys.append(data['key'])
                print(space_keys[space_count])
                space_count += 1
        
        print("\n SPACES FOUND: " + str(space_count) + "\n")


        for space_key in space_keys:
            self.scan_space(space_key, out)
    ...
    def _scan_page_content(self, page_data: dict, out):
        ...
        try:
            ...
            with subprocess.Popen(cli_cmd, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, shell=False) as cli_process:
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help='Scans all accessible spaces',
)
@click.option(
    '--pagination-size',
    type=click.INT,
...
def scan_confluence(url: str, page_id: Optional[int], space_key: Optional[str], scan_all: bool, pagination_size: int, output):
 
    username = USERNAME
    password = API_KEY

    if not username or not password:
        raise ClickException('Please set the ATLASSIAN_ACCOUNT_EMAIL and ATLASSIAN_API_TOKEN in "constants.py":\nUSERNAME=<email>\nAPI=<API key>')
    ...
    if page_id:
        ...
        scanner.scan_all_spaces(out=output)
    else:
        click.echo("Either --page-id, --space-key or --scan-all is required")
        raise Exit(code=1)
